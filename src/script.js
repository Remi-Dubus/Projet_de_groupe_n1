...
form.addEventListener("submit", (e) => {
  ...
  const restaurantAdress = e.target[2].value;
  const restaurantSpecialities = e.target[3].value;
  const restaurantPrice = e.target[4].value;
  const restaurantHour = e.target[5].value;
  const restaurantRating = e.target[6].value;
  const restaurantLink = e.target[7].value;
  const restaurantPicture = e.target[8].value;

  newRestaurant.push({
    picture: restaurantPicture,
    name: restaurantName,
    adresse: restaurantAdress,
    specialities: restaurantSpecialities,
    hour: restaurantHour,
...
// Faire en sorte qu'on puisse pas valider le tablo si tout n'est pas rempli
